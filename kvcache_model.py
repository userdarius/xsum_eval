...
class KVCacheModel:
    ...
    def _forward_with_kvcache(
        ...
        if self._past_key_values is None:
            ...
            if not_cached_logits.dim() == 2:
                not_cached_logits = torch.unsqueeze(not_cached_logits, 0)

            # Store raw logits
            self._logits_history = torch.cat(
                [self._logits_history, not_cached_logits], dim=1
            )

            # Calculate normalized probabilities
            not_cached_q = not_cached_logits.clone()
